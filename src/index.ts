...
import {mods, mod, modsloader} from './modsloader'
import {} from './modsloader'
...
export function apply(ctx: Context, cfg: Config) {
  ...
  var modsfunction = modsloader(ctx, cfg)
  ...
  .action(async ({session}, name, number) => {
    ...
    if (!name){
      ...
      if (!itemInfo){
        ...
        try {
          await ctx.monetary.cost(await ctx.idconverter.getUserAid(session.userId, session.platform), itemInfo.price.buy * modsfunction.numberYES(number))
    ...
    if (!name){
      ...
      if (!itemInfo){
        ...
        if (remove === ('Error: 你没有足够的该物品' || 'Error: 你没有该物品' || 'Error: 你没有足够的该建筑' || 'Error: 你没有该建筑' || 'Error: 你没有任何物品' || 'Error: 没有找到该物品')){
          ...
          try {
            await ctx.monetary.gain(await ctx.idconverter.getUserAid(session.userId, session.platform), itemInfo.price.sell * modsfunction.numberYES(number))
            return '成功卖出'
          } catch (e) {
            ctx.logger.error(e)
            return e
  ...
  .action(async ({session}, name, number) => {
    ...
    if (!name){
      ...
      if (!itemInfo){
        ...
        let item_Info = itemInfo.main.main as mod.crop
        ...
        for (let i = 0; i < nowHave.building.main.length; i++){
          let mod = modsfunction.inModfind(modsfunction.findName(nowHave.building.main[i].itemId)).main.main as mod.building
  ...
  .action(async ({session}) => {
    ...
    if (Crop_Growing.length === 0){
      ...
      for (let i = 0; i < Crop_Growing.length; i++){
        let output = []
        let cropInfo = modsfunction.inModfind(modsfunction.findName(Crop_Growing[i].crop_id)).main.main as mod.crop
  ...
  .action(async ({session}) => {
    ...
    modsfunction.mods.main.forEach(async (mod) => {
